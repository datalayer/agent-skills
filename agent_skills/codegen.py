...
def generate_skill_file(
    ...
    if params:
        ...
        for i, p in enumerate(params):
            ...
            if p.get("required", True):
                ...
                cli_args_code += "        print_usage()\n"
                cli_args_code += "        sys.exit(1)\n"
            else:
                default = p.get("default", "None")
    ...
    # Build usage string
    usage_params = " ".join(
        f"<{p['name']}>" if p.get("required", True) else f"[{p['name']}]" for p in params
    )
            ...
            "parameters": [
                {
                    "name": "check_command",
                    "type": "str",
                    "description": "Command to check condition",
                    "required": True,
                },
                {
                    "name": "expected_output",
                    "type": "str",
                    "description": "Expected output substring",
                    "required": True,
                },
                {
                    "name": "timeout",
