...
manager = SkillsManager(
    ...
    sandbox_variant="eval", # Sandbox type for execution
)
```
...
| `sandbox_variant` | `str` | `"eval"` | Sandbox for code execution |

`sandbox_variant` values come from `code-sandboxes` (for example: `eval`,
`jupyter`, `docker`, `colab`, `kaggle`, `modal`, `datalayer`, `monty`).
Cloud notebook notes:

- `colab` reuses an already-running Colab kernel (from explicit runtime values or a channels URL).
- `kaggle` can create a kernel with a Kaggle API token, or attach to an existing session.
